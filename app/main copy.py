...
def resolve_qa_eval_chain() -> Optional[Type]:
    ...
    for module_path in ("langchain.evaluation.qa", "langchain.evaluation"):
        try:
...
def render_chat_interface() -> None:
    ...
    try:
        ...
        with col2:
            num_results = st.slider("Related docs:", 1, 10, 5)
            ...
            if len(trimmed_question) < 20:
                ...
                with st.spinner("Analyzing your question..."):
                    try:
                        ...
                        results = similarity_search(merged_store, trimmed_question, k=num_results)
                        if results:
...
def main():
    ...
    st.title("📊 Capstone Sales Analysis")
    st.markdown("---")
    ...
    with st.sidebar:
        ...
        page = st.radio(
            "Select a page:",
            ["Home", "Vector Store", "Upload & Process", "Metrics", "LLMOps", "Analysis"]
        )
    ...
    if page == "Home":
        ...
        **Features:**
        - 📤 Upload and process sales data (PDF/CSV)
        - 📊 Build FAISS vector store for efficient search
        - 🔍 Semantic similarity search on your documents
        - 💬 Chat with your data using AI
        - 📈 Interactive data analysis with visualizations
        - 🎯 Ask questions about sales trends

        **Get Started:**
        1. Upload your sales data (PDF or CSV)
        2. Process and build vector embeddings
        3. Search and analyze your data
        4. Chat with AI about your findings
        """)
        ...
        try:
            ...
            with chat_tab:
                st.subheader("🤖 Intelligence Chat Assistant")
        ...
        with col1:
            st.write("**CSV Files** in `data/raw/`")
        ...
        with col2:
            st.write("**PDF Files** in `data/raw/PDF_Folder/`")
        ...
        st.markdown("---")
        st.subheader("📊 Vector Database Overview")
        ...
        try:
            ...
            if db_info:
                col1, col2, col3, col4 = st.columns(4)
                with col1:
                    st.metric("Vector Stores", db_info.get('total_indexes', 0))
                with col2:
                ...
                st.info("No vector database created yet. Go to 'Upload & Process' to create one.")
        except Exception as e:
            st.warning(f"Could not load database info: {e}")
        ...
        try:
            from src.database import list_indexes, load_vector_store
            
            indexes = list_indexes()
            if not indexes:
                st.info("No vector stores available. Upload data first.")
            else:
                selected_index = st.selectbox("Select vector store", indexes)
                ...
                if selected_index:
                    ...
                    if vector_store:
                        ...
                        if search_query and st.button("Search"):
                            ...
                            results = similarity_search(vector_store, search_query, k=num_results)
                        
                            if results:
                                st.success(f"Found {len(results)} matching documents")
                                for i, (doc, score) in enumerate(results, 1):
                                    with st.expander(f"Result {i} (Score: {score:.2f}):"):
                        ...
                        st.error(f"Could not load vector store: {selected_index}")
        ...
        eval_tab, viz_tab, assistant_tab = st.tabs([
            "✅ Model Evaluation",
            "📊 Visualizations",
            "💬 AI Assistant"
        ])
                                ...
                                with st.spinner("Running evaluation..."):
                                    ...
                                    if not vector_store:
                                        ...
                                        for i, grade in enumerate(graded):
                                            grade_value = grade.get("results") if isinstance(grade, dict) else grade
